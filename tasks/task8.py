...
- Генераторное выражение для фильтрации
...
def count_common_with_reverse(arr1, arr2):
    """Считает количество общих элементов с учётом перевёрнутых чисел (ФП-стиль)

    Использует генераторное выражение для ленивого перебора элементов arr1
    Не мутирует входные массивы

    Пример пайплайна:
        arr1 -> для каждого x -> проверка (x in arr2 или reverse(x) in arr2) -> подсчёт
